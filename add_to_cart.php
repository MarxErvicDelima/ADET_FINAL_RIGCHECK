...
if (isset($_GET['id'])) {
    ...
    if ($product) {
        ...
        $current_stock = getProductStock($product_id, 0);
        $already_in_cart = isset($_SESSION['cart'][$actual_variant_id]) ? $_SESSION['cart'][$actual_variant_id]['quantity'] : 0;
        
        if ($already_in_cart + $quantity > $current_stock) {
            $_SESSION['error_message'] = '⚠️ Sorry, only ' . $current_stock . ' units of ' . $product['product_name'] . ' are available in stock.';
