...
// Ensure stock is up-to-date from DB if available
foreach ($products as &$p) {
    $p['stock'] = getProductStock($p['product_id'], $p['stock']);
}
unset($p);
            ...
            <aside class="md:w-1/4 space-y-6">
                ...
                <div class="glass-card p-8 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <form method="GET" action="products.php" id="price-filter-form" class="space-y-4">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="text-sm text-gray-600 font-semibold mb-3 block">
                                    Price Range: <span class="text-primary">
                                        ₱<span id="min-display"><?php echo $min_price > 0 ? number_format($min_price, 0) : '0'; ?></span>
                                        – ₱<span id="max-display"><?php echo $max_price < 999999 ? number_format($max_price, 0) : '300,000'; ?></span>
                                    </span>
                                </label>

                                <!-- Fixed dual-range slider wrapper -->
                                <div class="price-slider-wrapper">
                                    <div id="slider-track-fill" class="slider-track-fill"></div>
                                    <input type="range" name="min_price" id="min-slider"
                                           min="0" max="300000" step="5000"
                                           value="<?php echo $min_price > 0 ? $min_price : 0; ?>">
                                    <input type="range" name="max_price" id="max-slider"
                                           min="0" max="300000" step="5000"
                                           value="<?php echo $max_price < 999999 ? $max_price : 300000; ?>">
                                </div>
                        ...
                        <style>
                            ...
                            /* Bring the active/last-touched slider on top */
                            #min-slider { z-index: 3; }
                            #max-slider { z-index: 2; }
                            #min-slider.on-top { z-index: 5; }
                            #max-slider.on-top { z-index: 5; }
                            ...
                            /* Hide webkit track (we use ::before pseudo + JS fill instead) */
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        <?php foreach ($products as $product): ?>
                            <div class="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100 group transition-all hover:shadow-2xl hover:border-primary animate-fade-in-up product-card flex flex-col">
                                <a href="product_details.php?id=<?php echo $product['product_id']; ?>" class="block relative overflow-hidden aspect-square bg-gray-100 flex-shrink-0">
                                    <img src="<?php echo htmlspecialchars(getProductImage($product['product_id'], $product['image_url'] ?? '')); ?>" alt="<?php echo htmlspecialchars($product['product_name'] ?? $product['name'] ?? 'Product'); ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy">
...
<script>
    ...
    if (minSlider && maxSlider && trackFill) {
        ...
        function updateSliders() {
            ...
            // Move the green fill track between the two thumbs
            const leftPct  = (minVal / rangeMax) * 100;
            const rightPct = 100 - (maxVal / rangeMax) * 100;
            trackFill.style.left  = leftPct  + '%';
            trackFill.style.right = rightPct + '%';
        }

        // Raise whichever thumb was last interacted with so it stays on top
        minSlider.addEventListener('pointerdown', () => {
            minSlider.classList.add('on-top');
            maxSlider.classList.remove('on-top');
        });
        maxSlider.addEventListener('pointerdown', () => {
            maxSlider.classList.add('on-top');
            minSlider.classList.remove('on-top');
        });

        minSlider.addEventListener('input', updateSliders);
        maxSlider.addEventListener('input', updateSliders);

        // No auto-submit — user must click "Apply Price Filter" to submit
